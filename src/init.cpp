...
void Shutdown()
{
    ...
    RenameThread("GainerCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: GainerCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: GainerCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeGainerCoinamount=<n> " + _("Keep N GainerCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. GainerCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. GainerCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("GainerCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "GainerCoin server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of GainerCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart GainerCoin to complete") << "\n";
    ...
    nAnonymizeGainerCoinAmount = GetArg("-anonymizeGainerCoinamount", 0);
    if(nAnonymizeGainerCoinAmount > 999999) nAnonymizeGainerCoinAmount = 999999;
    if(nAnonymizeGainerCoinAmount < 2) nAnonymizeGainerCoinAmount = 2;
    ...
    LogPrintf("Anonymize GainerCoin Amount %d\n", nAnonymizeGainerCoinAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1GNR+1000 == (.1GNR+100)*10
       10GNR+10000 == (1GNR+1000)*10
    */
