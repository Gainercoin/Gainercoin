...
int nAnonymizeGainerCoinAmount = 1000;
...
#else
    const char* pszModule = "GainerCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GainerCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GainerCoin
    // Mac: ~/Library/Application Support/GainerCoin
    // Unix: ~/.GainerCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GainerCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "GainerCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "GainerCoind.pid"));
